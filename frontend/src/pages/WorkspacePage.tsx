...
function Traffic({ data }: { data: Workspace }) {
  ...
  return <><SectionTitle eyebrow="TRAFFIC INJURY MODULE" title="交通事故人伤专业分析" description="整理事故、治疗、费用、赔偿证据与专业风险，不替代责任或医疗判断。" />
    ...
    <section className="panel"><div className="panel-head"><h3>专业风险提示</h3><span>{data.traffic_risks.length} 项</span></div><div className="cards-3">{data.traffic_risks.map(r => <article className="risk-card" key={r.id}><div><Badge tone={r.level === 'high' ? 'danger' : 'warning'}>{r.level === 'high' ? '高风险' : '中风险'}</Badge>{r.rule_id && <code>{r.rule_id}</code>}</div><h4>{r.risk_type}</h4><p>{r.description}</p><small>核查建议：{r.suggested_review}</small><SourceLinks sources={r.sources} /></article>)}</div></section>
...
function AgentRuns({ detail, runs, rerun }: { detail: Record<string, any> | null; runs: Array<Record<string, any>>; rerun: (node: string) => Promise<void> }) {
  ...
  return <><SectionTitle eyebrow="WORKFLOW OBSERVABILITY" title="Agent 执行过程" description="查看每个结构化节点的输入、输出、警告和重跑记录。" />
    <div className="run-summary observability-summary">
      <div><span>运行 ID</span><strong>{detail?.id?.slice(0, 8) || '—'}</strong></div>
      <div><span>状态</span><Badge tone={detail?.status === 'failed' ? 'danger' : detail?.status === 'awaiting_review' ? 'warning' : 'success'}>{detail?.status || '无运行'}</Badge></div>
      <div><span>Provider / Model</span><strong>{detail ? `${detail.provider} / ${detail.model_name}` : '—'}</strong></div>
      <div><span>端到端耗时</span><strong>{formatDuration(detail?.duration_ms)}</strong></div>
    ...
    <section className="panel workflow-list observability-list">{detail?.nodes?.map((node: Record<string, any>, index: number) => <article key={node.id} className={expanded === node.id ? 'expanded' : ''}>
      ...
      <div className="node-main">
        <div className="node-title"><strong>{node.node_name}</strong>{node.attempt > 1 && <Badge tone="info">重跑 #{node.attempt}</Badge>}</div>
        <p>生成 {node.output_summary?.generated_records || 0} 条结构化记录 · {formatDuration(node.duration_ms)} · 序号 {node.sequence}</p>
        {node.warnings?.map((w: string) => <small className="warning-text" key={w}>{w}</small>)}
        {node.error && <small className="node-error">{node.error}</small>}
        {expanded === node.id && <div className="node-inspector">
          <div><span>输入摘要</span><pre>{JSON.stringify(node.input_summary || {}, null, 2)}</pre></div>
          <div><span>输出摘要</span><pre>{JSON.stringify(node.output_summary || {}, null, 2)}</pre></div>
          <footer>开始：{node.started_at ? new Date(node.started_at).toLocaleString('zh-CN') : '—'} · 完成：{node.completed_at ? new Date(node.completed_at).toLocaleString('zh-CN') : '—'}{node.supersedes_node_run_id ? ` · 替代运行 ${node.supersedes_node_run_id.slice(0, 8)}` : ''}</footer>
        </div>}
      </div>
